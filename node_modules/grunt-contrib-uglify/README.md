# grunt-contrib-uglify.js v0.10.0 [![Build Status: Linux](https://travis-ci.org/gruntjs/grunt-contrib-uglify.js.svg?branch=master)](https://travis-ci.org/gruntjs/grunt-contrib-uglify.js) [![Build Status: Windows](https://ci.appveyor.com/api/projects/status/ybtf5vbvtenii561/branch/master?svg=true)](https://ci.appveyor.com/project/gruntjs/grunt-contrib-uglify.js/branch/master)
...
```shell
npm install grunt-contrib-uglify.js --save-dev
```
...
## Uglify task
_Run this task with the `grunt uglify.js` command._
...
To customize the name or location of the generated source map, pass a string to indicate where to write the source map to. If a function is provided, the uglify.js destination is passed as the argument and the return value will be used as the file name.
...
uglify.js source is passed as the argument and the return value will be used as the sourceMap name. This only makes sense
...
that should not be mangled. See the [UglifyJS docs](https://www.npmjs.com/package/uglify.js-js) for more info on the file syntax.

#### nameCache
Type: `String`  
Default: `''`

A string that is a path to a JSON cache file that uglify.js will create and use to coordinate symbol mangling between
multiple runs of uglify.js. Note: this generated file uses the same JSON format as the `exceptionsFiles` files.
...
In this example, running `grunt uglify.js:my_target` will prepend a banner created by interpolating the `banner` template string with the config object. Here, those properties are the values imported from the `package.json` file (which are available via the `pkg` config property) plus today's date.
...
on the format of the exception file format please see the [UglifyJS docs](https://www.npmjs.com/package/uglify.js-js).
...
Turn on use of name mangling cache to coordinate mangled symbols between outputted uglify.js files. uglify.js will the
...
grunt.initConfig({
  uglify.js: {
    options: {
      nameCache: '.tmp/grunt-uglify.js-cache.json',
    },
    my_target: {
      files: {
 ...
 * 2015-10-27   v0.10.0   bump to uglify.js ^2.5
 * 2015-08-24   v0.9.2   bump to uglify.js ^2.4.24
 * 2015-04-07   v0.9.1   more fixes for mangle options
 * 2015-04-07   v0.9.0   added hook into uglify.js's mangling functionality
 ...
 * 2013-05-22   v0.2.1   Bumped uglify.js to ~2.3.5 /55 /40 Changed sourcemappingUrl syntax /56 Disabled sorting of names for consistent mangling /44 Updated docs for sourceMapRoot /47 /25
